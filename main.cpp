...
{
public:
    virtual osgDB::ReaderWriter::ReadResult readObject(const std::string& filename, const osgDB::Options* options) override
    {
...
{
    osg::ref_ptr<osgDB::Options> osg_options = new osgDB::Options;
    osg_options->setReadFileCallback(new MyReadFileCallback);
    osgDB::Registry::instance()->setOptions(osg_options);
   ...
   // vsg::Path filename = "models/teapot.vsgt";
    vsg::Path filename = "OsgbTestData/Data/Data.dsm";
   ...
   // vsg::ref_ptr<vsg::Node> vsg_scene = vsg::ref_ptr<vsg::Node>(vsg::read(filename));
